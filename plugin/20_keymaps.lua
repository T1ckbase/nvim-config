...
vim.keymap.set('n', 'gO', function() MiniPick.registry.lsp_jump({ scope = 'document_symbol' }) end, { desc = 'Document symbols' })
...
vim.keymap.set('n', '<C-w>gri', function()
  vim.cmd.vsplit()
  MiniPick.registry.lsp_jump({ scope = 'implementation' })
end, { desc = 'Go to implementation in a new vertical split' })

vim.keymap.set('n', '<C-w>grr', function()
  vim.cmd.vsplit()
  MiniPick.registry.lsp_jump({ scope = 'references' })
